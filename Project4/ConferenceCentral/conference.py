...
SESSION_POST_CREATE_REQUEST = endpoints.ResourceContainer(
    SessionForm,
    websafeConferenceKey=messages.StringField(1)
)
...
@endpoints.api(name='conference', version='v1.1', audiences=[ANDROID_AUDIENCE],
...
# - - - Conference objects - - - - - - - - - - - - - - - - -
    ...
    def _getConferenceQuery(self, request):
        """Return formatted Conference query from the submitted filters."""
        q = Conference.query()
...
# - - - Session objects - - - - - - - - - - - - - -
    ...
    def _copySessionToForm(self, session):
        ...
        for field in sf.all_fields():
            ...
            # convert date or time to string; just copy others
                if field.name == 'date' or field.name.endswith('Time'):
    ...
    def _createSessionObject(self, request):
        ...
        # get conference for websafeConfKey
        conf = ndb.Key(urlsafe=request.websafeConferenceKey).get()
        ...
        # convert date from strings to Date objects; convert startTime to Time object
        if data['date']:
    ...
    @endpoints.method(SESSION_GET_BY_SPEAKER_REQUEST, SessionForms,
            path='conference/sessionsBySpeaker/{speaker}',
            http_method='GET', name='getSessionsBySpeaker')
    def getSessionsBySpeaker(self, request):
        """Return sessions with a certain speaker."""
        # make sure user is authed
        user = endpoints.get_current_user()
        if not user:
            raise endpoints.UnauthorizedException('Authorization required')
    ...
    @endpoints.method(SESSION_POST_CREATE_REQUEST, SessionForm,
...
# - - - Session Wishlist - - - - - - - - - - - - - - - - - - - - 
    ...
    @ndb.transactional(xg=True)
    def _sessionWishlist(self, request, wishlist=True):
        """Add or remove a session for user."""
        retval = None
        prof = self._getProfileFromUser() # get user Profile
    ...
    @endpoints.method(message_types.VoidMessage, SessionForms,
            path='sessionswishlisted',
            http_method='GET', name='getSessionsInWishlist')
    def getSessionsInWishlist(self, request):
        """Get list of sessions user has wishlisted."""
        prof = self._getProfileFromUser() # get user Profile
